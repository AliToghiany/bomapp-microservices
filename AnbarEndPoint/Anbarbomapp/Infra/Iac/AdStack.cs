        ...
        public AdStack()
        {
            ...
            WebApp webApp = new($"app-service-ad-{stackName}", new()
            {
                ...
                SiteConfig = new SiteConfigArgs
                {
                    ...
                    AppCommandLine = "dotnet Anbarbomapp.Server.Api.dll",
                    AppSettings = new()
                {
            ...
            Secret sqlDatabaseConnectionStringSecret = new(sqlDatabaseConnectionStringSecretName, new()
            {
                ...
                Properties = new SecretPropertiesArgs
                {
                    Value = Output.Tuple(sqlServer.Name, sqlDatabase.Name, sqlDatabaseDbUserPassword).Apply(t =>
                    {
                        ...
                        return $"Data Source=tcp:{_sqlServer}.database.windows.net;Initial Catalog={_sqlDatabase};User ID={sqlDatabaseDbUserId};Password={_sqlDatabasePassword};Application Name=Anbarbomapp;Encrypt=True;";
                    })
                }
            });
